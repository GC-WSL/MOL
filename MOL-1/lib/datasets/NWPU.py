...
class NWPU(imdb):
  def __init__(self, image_set, use_diff=False):
    name = 'NWPU_{}'.format(image_set)
    if use_diff:
      name += '_diff'
    imdb.__init__(self, name)
    self._image_set = image_set
    self._devkit_path = self._get_default_path()
    self._data_path = os.path.join(self._devkit_path, 'NWPU')
  ...
  def _do_python_eval(self, output_dir='output', use_corloc=False):
    ...
    for i, cls in enumerate(self._classes):
      ...
      if use_corloc:
        ...
        aps += [ap]
        recs += [rec[-1]]
        print(('AP for {} = {:.4f}, Recall =  {:.4f}'.format(cls, ap, rec[-1])))  
    ...
    if use_corloc:
      ...
      print(('Mean AP = {:.4f}, Mean Recall = {:.4f}'.format(np.mean(aps),np.mean(recs))))
